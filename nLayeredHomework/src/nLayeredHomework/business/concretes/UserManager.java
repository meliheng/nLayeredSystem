...
public class UserManager implements UserService {
	...
	public void add(User user) {
		...
		if(user.getPassword().length()<6 || !m.find() || user.getFirstName().length()<2 || user.getLastName().length()<2 || !checkEmailService.checkEmail(user, this)) {
			...
			if(emailVerificationService.verification(user.getEmail())) System.out.println("sisteme baþarýyla kayýt edildi");
			else System.out.println("email onayý yapýlamadý!");
		}
	...
	public void logIn(String email, String password) {
		if(userDao.get(email, password)) {
			...
			System.out.println("Yanlýþ parola veya email!");
		}
